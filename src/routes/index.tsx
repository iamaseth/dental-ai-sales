...
import { CalendarDays, MessageCircle, Phone, ArrowRight, X, Send } from "lucide-react";
...
function PatientSite() {
  ...
  function ask() {
    ...
    if (!q.trim()) return;
    if (/x-?ray|record|previous/.test(q)) setAnswer("Please tell me when your X-ray was taken, which tooth it concerns and what treatment was recommended. LaBelle may request new images after an examination.");
    else if (/pain|swelling|bleed|broken|emergency/.test(q)) setAnswer("Are you having trouble breathing or swallowing, uncontrolled bleeding or rapidly spreading swelling? If yes, seek emergency medical care now. Otherwise, tell me where the problem is, when it started and your pain level from 0–10.");
    else if (/book|appointment|visit/.test(q)) setAnswer("I can prepare an appointment request. Please share the treatment or problem, preferred date, your name, phone number and preferred language.");
    else if (/implant|brace|clean|filling|crown|root|veneer|service/.test(q)) setAnswer("LaBelle is publicly listed for orthodontics and dental implants. Patients also report cleanings, fillings, crowns, root-canal care and cosmetic work. The clinic must confirm availability and pricing.");
    else setAnswer("I do not have a clinic-approved answer for that yet, so I would send your question to LaBelle’s team instead of guessing. You can call 023 67 67 897.");
    setQuestion("");
  }
  ...
  return <main className="min-h-screen bg-[#fbfaf7] text-[#123f3b]">
    ...
    <header className="sticky top-0 z-30 border-b border-[#d7e7df] bg-[#fbfaf7]/95 backdrop-blur">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5">
        ...
        <nav className="hidden gap-7 text-sm font-semibold md:flex"><a href="#services">Services</a><a href="#appointments">Appointments</a><a href="#front-desk">Ask LaBelle</a><a href="#about">Contact</a></nav>
